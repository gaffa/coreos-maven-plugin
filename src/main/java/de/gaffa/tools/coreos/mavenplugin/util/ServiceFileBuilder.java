...
public class ServiceFileBuilder {
    ...
    private static File buildServiceFile(int instances, File baseDir, CoreOsUnit coreOsUnit, String dockerImageName, String dockerRunOptions, String xFleetOptions, String dockerHubUser, String dockerHubPass) throws IOException {

        File serviceFile = new File(baseDir.getAbsolutePath() + File.separator + coreOsUnit.getFullName());

        PrintWriter writer = new PrintWriter(serviceFile, "UTF-8");
        writer.println("[Unit]");
        writer.println("Description=" + coreOsUnit.getName() + " (File generated by coreos-maven-plugin)");
        writer.println("After=docker.service");
        writer.println("Requires=docker.service");
        writer.println("");
        writer.println("[Service]");
        writer.println("TimeoutStartSec=0");
        writer.println("ExecStartPre=-/usr/bin/docker kill " + coreOsUnit.getName());
        writer.println("ExecStartPre=-/usr/bin/docker rm " + coreOsUnit.getName());
        ...
        writer.println("ExecStart=/usr/bin/docker run " + dockerRunOptions + " --name=" + coreOsUnit.getName() + " " + dockerImageName);
        writer.println("ExecStop=/usr/bin/docker stop " + coreOsUnit.getName());
        writer.println("");
        ...
        if (instances > 1) {
            ...
            writer.println("Conflicts=" + coreOsUnit.getName() + ".*.service");
        }
        writer.close();
    ...
    public static List<File> build(int instances, String serviceName, String dockerImageName, String dockerRunOptions, String xFleetOptions, String dockerHubUser, String dockerHubPass) throws MojoExecutionException {
        ...
        for (int i = 0; i < instances; i++) {
            try {
                serviceFiles.add(buildServiceFile(instances, baseDir, new CoreOsUnit(serviceName, i + 1), dockerImageName, dockerRunOptions, xFleetOptions, dockerHubUser, dockerHubPass));
            } catch (IOException e) {
